...
def plot_tracking_with_class(image, tlwhs, obj_ids,cls_ids, class_names,scores=None, frame_id=0, fps=0., ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        intbox = tuple(map(int, (x1, y1, x1 + w, y1 + h)))
        obj_id = int(obj_ids[i])
        obj_cls = int(cls_ids[i])
        id_text = '{}'.format(int(obj_id))
        cls_text = '{}'.format(class_names[int(obj_cls)])
        if ids2 is not None:
            id_text = id_text + ', {}'.format(int(ids2[i]))
        _line_thickness = 1 if obj_id <= 0 else line_thickness
        id_color = get_color(abs(obj_id))
        # cls_color = get_color(abs(obj_cls))
        cv2.rectangle(im, intbox[0:2], intbox[2:4], color=id_color, thickness=line_thickness)

        cv2.putText(im, cls_text, (intbox[0], intbox[1]-5), cv2.FONT_HERSHEY_PLAIN, text_scale,(0, 255, 0),
                    thickness=text_thickness)      
        cv2.putText(im, id_text, (intbox[0], intbox[1] + 20), cv2.FONT_HERSHEY_PLAIN, text_scale*1.2, (0, 0, 255),
                    thickness=text_thickness)
