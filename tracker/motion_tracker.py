...
from .matching import fuse_classes_width_height, iou_distance, linear_assignment, fuse_score, motion_distance
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, classes):
        ...
        self.score = score
        self.tracklet_len = 0
        # Multiple  class MOT, adding a class attribute
...
class MotionTracker(object):
    ...
    def update_with_motion(self, output_results, img_info, img_size):
        self.frame_id += 1
        ...
        r_tracked_stracks = [strack_pool[i] for i in u_track]  # low score detection boxes can also be associated with lost stracks
        ...
        if self.use_motion:
            ...
            r_r_tracked_stracks = [r_tracked_stracks[i]
                                   for i in u_track]  # Gaussian distance can also be associated with lost stracks

            # Get the Gaussian distance
            dists = motion_distance(self.kalman_filter, r_r_tracked_stracks,
                                    detections, motion_thresh=self.motion_thresh) 
            dists = fuse_classes_width_height(dists, r_r_tracked_stracks, detections) 

            matches, u_track, u_detection = linear_assignment(
                dists, thresh=self.motion_match_thresh)
            for itracked, idet in matches:
        ...
        dists = fuse_classes_width_height(dists, unconfirmed, detections)  
        dists = fuse_score(dists, detections)
        ...
        if self.use_motion:
            ...
            dists = motion_distance(self.kalman_filter, r_unconfirmed,
                                    detections, motion_thresh=self.motion_thresh) 
